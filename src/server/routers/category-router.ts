...
export const categoryRouter = router({
  getEventCategories: privateProcedure.query(async ({ c, ctx }: any) => {
    ...
    const categoriesWithCounts = await Promise.all(
      categories.map(async (category) => {
        ...
        const [uniqueFieldsCount, eventsCount, lastPing] = await Promise.all([
          db.event
            .findMany({
              where: { id: category.id, createdAt: { gte: firstDayOfMonth } },
              select: { fields: true },
              distinct: ["fields"],
            })
            .then((events) => {
    ...
    // superjason know how to deal with dates, map,set and bigInt
  ...
  getEventByCategoryName: privateProcedure
    .input(
      z.object({
        ...
        timeRange: z.enum(["today", "month", "week"]),
      })
    )
    .query(async ({ c, ctx, input }) => {
